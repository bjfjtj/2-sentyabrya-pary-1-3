# Задача 8 – Расширение: Triangle

## Условие
Не меняя код цикла из Задачи 7 (ни одной строчки!), создайте новый класс `Triangle`, унаследованный от `Figure`, переопределите в нём метод `draw()` и добавьте объект треугольника в тот же самый список. Запустите программу.

Цель – показать, что полиморфизм позволяет легко расширять программу новыми типами без переписывания старой логики (без добавления новых `if/else` или циклов).

## Решение
Добавляется класс `Triangle` с переопределённым методом `draw()`. Объект треугольника добавляется в существующий список. Цикл остаётся без изменений и корректно обрабатывает новый тип.
